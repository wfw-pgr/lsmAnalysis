...
contains
  ...
  subroutine magneticField__triPrism( vertex, bfdpos, mvec, zp, bfield, nSubdiv, nDiv_z )
    ...
    ! ------------------------------------------------------ !
    call single__subdivide( vertex(:,v1_), vertex(:,v2_), vertex(:,v3_), subdivE, nSubdiv, nSubE )
  ...
  ! ====================================================== !
  recursive subroutine single__subdivide( node1, node2, node3, node_ret, nSubdiv, node_len )
    implicit none
    integer         , parameter     :: dim=3, nVert=3
    integer         , intent(in)    :: nSubdiv, node_len
    ...
    else if ( nSubdiv.ge.1 ) then
       node12(:) = 0.5d0 * ( node1(:) + node2(:) )
       node23(:) = 0.5d0 * ( node2(:) + node3(:) )
       node31(:) = 0.5d0 * ( node3(:) + node1(:) )
       sect_len  = node_len / 4
       call single__subdivide( node1 , node12, node31, node_ret(:,:,(           1):(1*sect_len) ), nSubdiv-1, sect_len )
       call single__subdivide( node2 , node23, node12, node_ret(:,:,(  sect_len+1):(2*sect_len) ), nSubdiv-1, sect_len )
       call single__subdivide( node3 , node31, node23, node_ret(:,:,(2*sect_len+1):(3*sect_len) ), nSubdiv-1, sect_len )
       call single__subdivide( node12, node23, node31, node_ret(:,:,(3*sect_len+1):(4*sect_len) ), nSubdiv-1, sect_len )
    endif
